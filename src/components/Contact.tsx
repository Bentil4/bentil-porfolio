...
export function Contact() {
  ...
  return (
    <Section id="contact" className="border-t border-line">
      <div className={`grid gap-10 ${hasForm ? 'lg:grid-cols-[1fr_1.2fr] lg:gap-16' : ''}`}>
        <div>
          <Reveal>
            <SectionHeading
              id="contact-heading"
          ...
          <Reveal delay={0.08}>
            <ul className={`grid gap-4 ${hasForm ? '' : 'sm:grid-cols-2 lg:max-w-3xl'}`}>
              <li>
                <ContactCard href={`mailto:${email}`} label="Email" value={email} icon={<Mail size={20} aria-hidden="true" />} />
              </li>
              {socials.linkedin && (
                <li>
                  <ContactCard
