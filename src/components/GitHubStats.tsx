...
const THEME =
  "bg_color=12141a&title_color=c6ff3d&icon_color=5b9dff&text_color=eef1f6&hide_border=true";
...
const liveUrls = (user: string, statsApi?: string) => {
  ...
  return {
    stats:
      api &&
      `${api}/api?username=${user}&show_icons=true&include_all_commits=true&${THEME}`,
    langs:
      api &&
      `${api}/api/top-langs/?username=${user}&layout=donut&langs_count=6&${THEME}`,
...
export function GitHubStats() {
  ...
  const urls =
    enableLiveStats && username
      ? liveUrls(encodeURIComponent(username), statsApiUrl)
      : null;
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Reveal delay={0.08} className="h-full">
          <Panel title="Top Languages">
            <LiveOrPlaceholder
              src={urls?.langs}
              alt={`Top languages used by ${username}`}
              placeholder={<LanguagesSkeleton />}
            />
          </Panel>
        </Reveal>
...
const Bar = ({ className }: { className: string }) => (
  <span className={`block animate-shimmer rounded bg-white/10 ${className}`} />
);
...
function StatsSkeleton() {
  ...
  return (
    <div
      role="img"
      aria-label="GitHub stats widget placeholder"
    ...
    >
      <ul className="flex-1 space-y-3.5">
        {rows.map((row, i) => (
          <li
            key={row}
            className="flex items-center justify-between gap-4 font-mono text-xs text-muted"
          >
            ...
            <Bar
              className={`h-3 ${["w-10", "w-14", "w-8", "w-6", "w-9"][i]}`}
            />
          </li>
        ))}
      </ul>
...
function LanguagesSkeleton() {
  ...
  let acc = 0;
  const stops = langs
    .map((l) => `${l.color} ${acc}% ${(acc += l.pct)}%`)
    .join(", ");

  return (
    <div
      role="img"
      aria-label="Top languages chart placeholder"
      className="flex w-full flex-col items-center gap-6 sm:flex-row"
    >
      <div
        className="relative size-32 shrink-0 rounded-full opacity-80"
    ...
    >
      <div className="grid w-full grid-flow-col grid-rows-7 gap-[2px] sm:gap-[3px]">
        {CELLS.map((level, i) => (
          <span
            key={i}
            className={`aspect-square rounded-[2px] ${LEVELS[level]}`}
          />
        ))}
      </div>
