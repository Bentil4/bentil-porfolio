...
export const portfolio = {
  ...
  email: "broderick.bentil@gmail.com", // EDIT
  resumeUrl: "https://drive.google.com/your-resume-link", // EDIT: external resume link

  socials: {
    github: "https://github.com/Bentil4",
    linkedin: "https://www.linkedin.com/in/broderickbentil",
  },
  ...
  // Each card follows the STAR framework.
  projects: [
    {
      ...
      star: {
        situation:
          "A SaaS team relied on nightly CSV exports, so product decisions lagged real usage by a full day.",
        task: "Build a real-time analytics dashboard that non-technical stakeholders could self-serve.",
        action:
          "Designed an event pipeline with PostgreSQL + materialized views, a typed GraphQL API, and a Next.js dashboard with streaming charts.",
        result:
          "Cut time-to-insight from 24h to under 5s and removed ~6 hours/week of manual reporting.",
      },
    ...
    },
    {
      ...
      star: {
        situation:
          "Small Node services needed background jobs, but existing queues pulled in Redis and heavy dependencies.",
        task: "Create a zero-dependency, Postgres-backed job queue with retries and scheduling.",
        action:
          "Implemented SKIP LOCKED polling, exponential backoff, and a typed job API with 95% test coverage and CI on every PR.",
        result:
          "Adopted by 40+ repos; one team replaced Redis entirely and reduced infra cost by 18%.",
      },
    ...
    },
    {
      ...
      star: {
        situation:
          "A multi-location clinic lost bookings because its legacy site was slow and unusable on mobile.",
        task: "Rebuild the booking flow as a fast, accessible, mobile-first web app.",
        action:
          "Built a React SPA with optimistic UI, form validation, and WCAG AA components; code-split routes and lazy-loaded heavy widgets.",
        result:
          "Lighthouse performance 42 → 98, and online bookings rose 31% in the first quarter.",
      },
